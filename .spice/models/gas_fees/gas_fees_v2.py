...
class XGBoostForecaster:
    ...
    def infer(self, data: pd.DataFrame) -> np.array:
        input_window = data['value'][-self.lookback_size:
                                     ].to_numpy().reshape(-1).astype("float32")
        if 'covariate' in data.columns:
            covariates = data['covariate'][-self.lookback_size:
                                           ].to_numpy().reshape(-1).astype("float32")
            input_window = np.concatenate(
                [input_window, covariates], axis=None)
...
def _make_inference_response(predicted: np.ndarray, now: float) -> spec.InferenceResponse:
    return spec.InferenceResponse(forecast=[
        spec.InferencePoint(now + i + 1, float(val)
                            if not math.isnan(float(val)) else -1e10, None)
...
def infer(context: Any, runtime: Any) -> None:
    params = spec.InferenceParams(lookback=[spec.InferencePoint(
        **x) for x in context.pop('lookback')], **context)
    forecaster = XGBoostForecaster(params.lookback_size, params.forecast_size)
    forecaster.load_model(os.path.join(
        os.getenv('MODEL_DIR'), params.model_weights_name))

    prediction = forecaster.infer(pd.DataFrame(
        [asdict(x) for x in params.lookback]))
